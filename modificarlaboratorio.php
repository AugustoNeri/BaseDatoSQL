...
?>
    <html>
    <head><title>Moficar Laboratorio</title></head>
    <body>
...
<th><font color=blue size=5>ID_Laboratorio</th>
<th><font color=blue size=5>Laboratorio</th>
...
while ($row=mysql_fetch_array($resultado)){
	echo "<tr><td align=center>".$row['ID_LAB']."</td>";
	echo "<td align=center>".$row['NOM_LAB']."</td>";
echo "<td align=center><a href=".$_SERVER['PHP_SELF']."?cambiar=".$row['ID_LAB'].">Editar</a></td></tr>";
}
?>
	</table>
	</div>
...
if (isset ($cambiar)){
	...
	$sql = "SELECT * FROM ".$tabla." WHERE ID_LAB=".$cambiar;
	//echo "sql=".$sql;
	...
	<p><font color=blue size=5>ID_Laboratorio:
	<input type="hidden" align="LEFT" name="id_lab" value="<?php echo $registro['ID_LAB'];?>" /><p>
	<p><font color=blue size=5>Laboratorio:
	<input type="text" align="LEFT" name="nom_lab" value="<?php echo $registro['NOM_LAB'];?>"/><p>
