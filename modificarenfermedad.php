...
?>
    <html>
    <head><title>Modificar Enfermedad</title></head>
    <body>
...
<th><font color=blue size=5>ID_Enfermedad</th>
<th><font color=blue size=5>Enfermedad</th>
...
while ($row=mysql_fetch_array($resultado)){
	echo "<tr><td align=center>".$row['ID_EMF']."</td>";
	echo "<td align=center>".$row['NOM_EMF']."</td>";
echo "<td align=center><a href=".$_SERVER['PHP_SELF']."?cambiar=".$row['ID_EMF'].">Editar</a></td></tr>";
}
?>
	</table>
	</div>
...
if (isset ($cambiar)){
	...
	$sql = "SELECT * FROM ".$tabla." WHERE ID_EMF=".$cambiar;
	//echo "sql=".$sql;
	...
	<p><font color=blue size=5>ID_Enfermedad:
	<input type="hidden" align="LEFT" name="id_emf" value="<?php echo $registro['ID_EMF'];?>" /><p>
	<p><font color=blue size=5>Enfermedad:
	<input type="text" align="LEFT" name="nom_emf" value="<?php echo $registro['NOM_EMF'];?>"/><p>
