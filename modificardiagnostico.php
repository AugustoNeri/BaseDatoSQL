...
?>
    <html>
    <head><title>Modificar Diagnóstico</title></head>
    <body>
...
<th><font color=blue size=5>ID_Diagnóstico</th>
<th><font color=blue size=5>Diagnóstico</th>
...
while ($row=mysql_fetch_array($resultado)){
	echo "<tr><td align=center>".$row['ID_DIAG']."</td>";
	echo "<td align=center>".$row['NOM_DIAG']."</td>";
echo "<td align=center><a href=".$_SERVER['PHP_SELF']."?cambiar=".$row['ID_DIAG'].">Editar</a></td></tr>";
}
?>
	</table>
	</div>
...
if (isset ($cambiar)){
	...
	$sql = "SELECT * FROM ".$tabla." WHERE ID_DIAG=".$cambiar;
	//echo "sql=".$sql;
	...
	<p><font color=blue size=5>ID_Diagnóstico:
	<input type="hidden" align="LEFT" name="id_diag" value="<?php echo $registro['ID_DIAG'];?>" /><p>
	<p><font color=blue size=5>Diagnóstico:
	<input type="text" align="LEFT" name="nom_diag" value="<?php echo $registro['NOM_DIAG'];?>"/><p>
