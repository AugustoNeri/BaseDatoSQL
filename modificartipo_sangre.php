...
?>
    <html>
    <head><title>Modificar Tipo_Sangre</title></head>
    <body>
...
<th><font color=blue size=5>ID_Tipo Sangre</th>
<th><font color=blue size=5>Tipo de Sangre</th>
...
while ($row=mysql_fetch_array($resultado)){
	echo "<tr><td align=center>".$row['ID_TS']."</td>";
	echo "<td align=center>".$row['NOM_TS']."</td>";
echo "<td align=center><a href=".$_SERVER['PHP_SELF']."?cambiar=".$row['ID_TS'].">Editar</a></td></tr>";
}
?>
	</table>
	</div>
...
if (isset ($cambiar)){
	...
	$sql = "SELECT * FROM ".$tabla." WHERE ID_TS=".$cambiar;
	//echo "sql=".$sql;
	...
	<p><font color=blue size=5>ID_Tipo Sangre:
	<input type="hidden" align="LEFT" name="id_ts" value="<?php echo $registro['ID_TS'];?>" /><p>
	<p><font color=blue size=5>Tipo de Sangre:
	<input type="text" align="LEFT" name="nom_ts" value="<?php echo $registro['NOM_TS'];?>"/><p>
