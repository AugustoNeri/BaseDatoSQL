...
?>
    <html>
    <head><title>Modificar Tipo_Terapia</title></head>
    <body>
...
<th><font color=blue size=5>ID_Tipo Terapia</th>
<th><font color=blue size=5>Tipo de Terapia</th>
...
while ($row=mysql_fetch_array($resultado)){
	echo "<tr><td align=center>".$row['ID_TPR']."</td>";
	echo "<td align=center>".$row['NOM_TPR']."</td>";
echo "<td align=center><a href=".$_SERVER['PHP_SELF']."?cambiar=".$row['ID_TPR'].">Editar</a></td></tr>";
}
?>
	</table>
	</div>
...
if (isset ($cambiar)){
	...
	$sql = "SELECT * FROM ".$tabla." WHERE ID_TPR=".$cambiar;
	//echo "sql=".$sql;
	...
	<p><font color=blue size=5>ID_Tipo Terapia:
	<input type="hidden" align="LEFT" name="id_tpr" value="<?php echo $registro['ID_TPR'];?>" /><p>
	<p><font color=blue size=5>Tipo de Terapia:
	<input type="text" align="LEFT" name="nom_tpr" value="<?php echo $registro['NOM_TPR'];?>"/><p>
