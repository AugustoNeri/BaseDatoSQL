...
?>
    <html>
    <head><title>Modificar Tipo_Enfermedad</title></head>
    <body>
...
<th><font color=blue size=5>ID_Tipo Enfermedad</th>
<th><font color=blue size=5>Tipo de Enfermedad</th>
...
while ($row=mysql_fetch_array($resultado)){
	echo "<tr><td align=center>".$row['ID_TE']."</td>";
	echo "<td align=center>".$row['NOM_TE']."</td>";
echo "<td align=center><a href=".$_SERVER['PHP_SELF']."?cambiar=".$row['ID_TE'].">Editar</a></td></tr>";
}
?>
	</table>
	</div>
...
if (isset ($cambiar)){
	...
	$sql = "SELECT * FROM ".$tabla." WHERE ID_TE=".$cambiar;
	//echo "sql=".$sql;
	...
	<p><font color=blue size=5>ID_Tipo Enfermedad:
	<input type="hidden" align="LEFT" name="id_te" value="<?php echo $registro['ID_TE'];?>" /><p>
	<p><font color=blue size=5>Tipo de Enfermedad:
	<input type="text" align="LEFT" name="nom_te" value="<?php echo $registro['NOM_TE'];?>"/><p>
