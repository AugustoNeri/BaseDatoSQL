...
?>
    <html>
    <head><title>Modificar Presentación</title></head>
    <body>
...
<th><font color=blue size=5>ID_Presentación</th>
<th><font color=blue size=5>Presentación</th>
...
while ($row=mysql_fetch_array($resultado)){
	echo "<tr><td align=center>".$row['ID_PRE']."</td>";
	echo "<td align=center>".$row['NOM_PREN']."</td>";
echo "<td align=center><a href=".$_SERVER['PHP_SELF']."?cambiar=".$row['ID_PRE'].">Editar</a></td></tr>";
}
?>
	</table>
	</div>
...
if (isset ($cambiar)){
	...
	$sql = "SELECT * FROM ".$tabla." WHERE ID_PRE=".$cambiar;
	//echo "sql=".$sql;
	...
	<p><font color=blue size=5>ID_Presentación:
	<input type="hidden" align="LEFT" name="id_pre" value="<?php echo $registro['ID_PRE'];?>" /><p>
	<p><font color=blue size=5>Presentación:
	<input type="text" align="LEFT" name="nom_pre" value="<?php echo $registro['NOM_PREN'];?>"/><p>
