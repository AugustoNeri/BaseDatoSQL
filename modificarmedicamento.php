...
?>
    <html>
    <head><title>Modificar Medicamento</title></head>
    <body>
...
<th><font color=blue size=5>ID_Medicamento</th>
<th><font color=blue size=5>Medicamento</th>
...
while ($row=mysql_fetch_array($resultado)){
	echo "<tr><td align=center>".$row['ID_MED']."</td>";
	echo "<td align=center>".$row['NOM_MED']."</td>";
echo "<td align=center><a href=".$_SERVER['PHP_SELF']."?cambiar=".$row['ID_MED'].">Editar</a></td></tr>";
}
?>
	</table>
	</div>
...
if (isset ($cambiar)){
	...
	$sql = "SELECT * FROM ".$tabla." WHERE ID_MED=".$cambiar;
	//echo "sql=".$sql;
	...
	<p><font color=blue size=5>ID_Medicamento:
	<input type="hidden" align="LEFT" name="id_med" value="<?php echo $registro['ID_MED'];?>" /><p>
	<p><font color=blue size=5>Medicamento:
	<input type="text" align="LEFT" name="nom_med" value="<?php echo $registro['NOM_MED'];?>"/><p>
